import React from 'react';
import { Download, TrendingDown, Clock, Hash } from 'lucide-react';
import SearchableTable from '../../components/general/SearchableTable';
import { useNavigate } from 'react-router-dom';

interface Withdrawal {
  id: string;
  date: string;
  reference: string;
  method: string;
  destination: string;
  amount: number;
  status: 'pending' | 'processing' | 'completed' | 'failed' | 'cancelled';
  fees: number;
  netAmount: number;
}

const WithdrawalsHistory: React.FC = () => {
  const navigate = useNavigate();

  const withdrawals: Withdrawal[] = [
    {
      id: 'WTH001',
      date: '2025-01-15 14:30',
      reference: 'WTH-2025-001',
      method: 'Bank Transfer',
      destination: '****1234',
      amount: 2000,
      status: 'completed',
      fees: 20,
      netAmount: 1980,
    },
    {
      id: 'WTH002',
      date: '2025-01-14 10:15',
      reference: 'WTH-2025-002',
      method: 'Mobile Money',
      destination: '+1 555-****-0123',
      amount: 500,
      status: 'completed',
      fees: 5,
      netAmount: 495,
    },
    {
      id: 'WTH003',
      date: '2025-01-13 09:45',
      reference: 'WTH-2025-003',
      method: 'Crypto Transfer',
      destination: '1A2B3C****5G8H9I0J',
      amount: 5000,
      status: 'processing',
      fees: 0,
      netAmount: 5000,
    },
    {
      id: 'WTH004',
      date: '2025-01-12 16:20',
      reference: 'WTH-2025-004',
      method: 'Bank Transfer',
      destination: '****5678',
      amount: 1500,
      status: 'pending',
      fees: 15,
      netAmount: 1485,
    },
    {
      id: 'WTH005',
      date: '2025-01-11 11:30',
      reference: 'WTH-2025-005',
      method: 'Cash Pickup',
      destination: 'New York Branch',
      amount: 1000,
      status: 'failed',
      fees: 10,
      netAmount: 0,
    },
  ];

  // const filteredWithdrawals = withdrawals;

  const getStatusColor = (status: string) => {
    switch (status) {
      case 'completed':
        return 'bg-green-100 text-green-800';
      case 'processing':
        return 'bg-blue-100 text-blue-800';
      case 'pending':
        return 'bg-yellow-100 text-yellow-800';
      case 'failed':
        return 'bg-red-100 text-red-800';
      case 'cancelled':
        return 'bg-gray-100 text-gray-800';
      default:
        return 'bg-gray-100 text-gray-800';
    }
  };

  const getStatusLabel = (status: string) => {
    return status ? status.charAt(0).toUpperCase() + status.slice(1) : 'Unknown';
  };

  const totalWithdrawn = withdrawals
    .filter((w) => w.status === 'completed')
    .reduce((sum, w) => sum + w.amount, 0);

  const totalFees = withdrawals
    .filter((w) => w.status === 'completed')
    .reduce((sum, w) => sum + w.fees, 0);

  const pendingCount = withdrawals.filter((w) => w.status === 'pending').length;

  // Define table columns
  const columns = [
    {
      accessor: 'date',
      title: 'Date & Time',
      sortable: true,
    },
    {
      accessor: 'reference',
      title: 'Reference',
      sortable: true,
      render: (_: any, record: Withdrawal) => (
        <span className="font-medium text-gray-900">{record.reference}</span>
      ),
    },
    {
      accessor: 'method',
      title: 'Method',
      sortable: true,
      render: (_: any, record: Withdrawal) => (
        <span className="text-gray-600">{record.method}</span>
      ),
    },
    {
      accessor: 'destination',
      title: 'Destination',
      render: (_: any, record: Withdrawal) => (
        <span className="text-gray-600 font-mono text-sm">{record.destination}</span>
      ),
    },
    {
      accessor: 'amount',
      title: 'Amount',
      sortable: true,
      textAlignment: 'right',
      render: (_: any, record: Withdrawal) => (
        <span className="font-semibold text-gray-900">
          ${record.amount?.toLocaleString() || '0'}
        </span>
      ),
    },
    {
      accessor: 'fees',
      title: 'Fees',
      sortable: true,
      textAlignment: 'right',
      render: (_: any, record: Withdrawal) => (
        <span className="text-gray-600">${record.fees?.toFixed(2) || '0.00'}</span>
      ),
    },
    {
      accessor: 'netAmount',
      title: 'Net Amount',
      sortable: true,
      textAlignment: 'right',
      render: (_: any, record: Withdrawal) => (
        <span className="font-semibold text-gray-900">
          ${record.netAmount?.toLocaleString() || '0'}
        </span>
      ),
    },
    {
      accessor: 'status',
      title: 'Status',
      sortable: true,
      textAlignment: 'center',
      render: (_: any, record: Withdrawal) => (
        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(record.status)}`}>
          {getStatusLabel(record.status)}
        </span>
      ),
    },
  ];

  // Define table actions
  const actions = [
    {
      label: 'View',
      onClick: (record: Withdrawal) => {
        console.log('View withdrawal:', record);
      },
    },
  ];

  // Define filters
  const filters = [
    {
      key: 'status',
      label: 'Status',
      options: [
        { value: 'all', label: 'All Statuses' },
        { value: 'pending', label: 'Pending' },
        { value: 'processing', label: 'Processing' },
        { value: 'completed', label: 'Completed' },
        { value: 'failed', label: 'Failed' },
        { value: 'cancelled', label: 'Cancelled' },
      ],
      defaultValue: 'all',
    },
  ];

  return (
    <div className="min-h-screen bg-gray-50">
      <div className="flex gap-2 overflow-x-hidden text-sm pl-6">
        <button
          className="text-gray-500 hover:cursor-pointer hover:text-gray-800 transition-colors"
          onClick={() => navigate('/dashboard')}
        >
          Home
        </button>
        <span className="text-gray-400">/</span>
        <button
          className="text-gray-500 hover:cursor-pointer hover:text-gray-800 transition-colors"
        >
          Transactions
        </button>
        <span className="text-gray-400">/</span>
        <span className="text-gray-700 font-semibold">
          Withdrawal History
        </span>
      </div>

      {/* Header */}
      <div className="sticky top-0 z-10">
        <div className="max-w-8xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-semibold text-gray-800">Withdrawal History</h1>
            <button className="px-4 py-2 bg-blue-600 text-white rounded-full hover:bg-blue-700 flex items-center gap-2 transition-colors">
              <Download className="w-4 h-4" />
              Export
            </button>
          </div>
        </div>
      </div>

      <div className="max-w-8xl mx-auto px-6 py-8">
        {/* Summary Cards */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Total Withdrawn</p>
                <p className="text-2xl font-bold text-red-600">${totalWithdrawn.toLocaleString()}</p>
                <p className="text-xs text-gray-500 mt-1">{withdrawals.filter(w => w.status === 'completed').length} completed</p>
              </div>
              <div className="p-3 bg-red-100 rounded-lg">
                <TrendingDown className="h-6 w-6 text-red-600" />
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Pending</p>
                <p className="text-2xl font-bold text-yellow-600">{pendingCount}</p>
                <p className="text-xs text-gray-500 mt-1">{withdrawals.length > 0 ? Math.round((pendingCount / withdrawals.length) * 100) : 0}% of total</p>
              </div>
              <div className="p-3 bg-yellow-100 rounded-lg">
                <Clock className="h-6 w-6 text-yellow-600" />
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Total Fees</p>
                <p className="text-2xl font-bold text-blue-600">${totalFees.toFixed(2)}</p>
                <p className="text-xs text-gray-500 mt-1">{totalWithdrawn > 0 ? ((totalFees / totalWithdrawn) * 100).toFixed(2) : 0}% of withdrawals</p>
              </div>
              <div className="p-3 bg-blue-100 rounded-lg">
                <Download className="h-6 w-6 text-blue-600" />
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Total Count</p>
                <p className="text-2xl font-bold text-purple-600">{withdrawals.length}</p>
                <p className="text-xs text-gray-500 mt-1">All transactions</p>
              </div>
              <div className="p-3 bg-purple-100 rounded-lg">
                <Hash className="h-6 w-6 text-purple-600" />
              </div>
            </div>
          </div>
        </div>

        {/* SearchableTable */}
        <SearchableTable
          data={withdrawals}
          columns={columns}
          actions={actions}
          filters={filters}
          searchPlaceholder="Search by reference, method, or destination..."
          highlightOnHover
          recordsPerPageOptions={[5, 10, 25, 50]}
          defaultRecordsPerPage={10}
        />
      </div>
    </div>
  );
};

export default WithdrawalsHistory;
